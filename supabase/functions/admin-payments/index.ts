...
Deno.serve(async(req:Request)=>{
 ...
 if(req.method==="POST"){
  ...
  const {data,error}=await admin.rpc("record_manual_membership_payment",{p_organization_id:organizationId,p_membership_id:membershipId,p_method:method,p_reference_number:reference||null,p_received_by:auth.user.id});await admin.from("audit_logs").insert({organization_id:organizationId,actor_user_id:auth.user.id,action:"payment.register",entity_type:"payment_records",entity_id:data,details:{method}});if(error)return json(req,{error:error.message},error.code==="23505"?409:400);return json(req,{ok:true,id:data},201);
 }
 if(req.method==="PATCH"){
  ...
  const {error}=await admin.rpc("void_manual_membership_payment",{p_organization_id:organizationId,p_payment_id:paymentId,p_voided_by:auth.user.id,p_void_reason:reason});await admin.from("audit_logs").insert({organization_id:organizationId,actor_user_id:auth.user.id,action:"payment.void",entity_type:"payment_records",entity_id:paymentId,details:{reason}});if(error)return json(req,{error:error.message},400);return json(req,{ok:true});
 }
